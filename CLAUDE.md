...
2. M3 is complete (2026-09-19): F-5.6 scene summaries, F-14.11 beta reader, F-5.7 query mode (lexical retrieval over summaries, verified citations; embeddings are a later feature), F-5.13 background job queue (migration `0007_index_jobs`, `src/main/jobs/`, the `jobs:*` channels; the spec is amended: no provider batch API at launch, only `summary` jobs), F-5.8 rename modes (Query/Author/Plan), F-5.9 cost visibility (`describeRequest`, `ChatMessage.usage`, `src/main/ai/sessionUsage.ts`), F-4.10 tag filter (`documentTag:listAll`, `tagFilter` on the tree store, `TagFilterBar`, the documents list and "Show in tree" in `TagDetail`). Next: the launch line, F-15.10 (site live) in the `cloud-website` worktree at `~/coding/mythscribe-cloud` (fully merged into main), then M-Cloud (F-15.2 onward); decide F-7.4 with the user first. Read `PLAN.md` §2 and §3 before anything under §2.14.
