...
2. M1 is under way: F-1.1 through F-1.5, F-2.1 through F-2.6, F-3.1 through F-3.5, and F-3.8 are done. Continue with `/feature F-3.6` (formatting settings: `settings:get`/`settings:set` channels for the `editor` key, applied live through CSS variables on the editing pane, of which `--ms-editor-max-width` from `column.ts` is the first; the Settings dialog UI itself is F-7.5), then F-3.7 (notes), then F-7.2, F-7.3, F-7.5 (editor tab) to close M1. v0 import was split out as F-1.6 (post-launch).
