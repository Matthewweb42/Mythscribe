...
2. Next: F-14.3 scene brief (replaces the notes-as-brief stand-in in `critique.v1` and ghost text; the voice block already carries the F-14.2 author rules, so every prose prompt gets them for free), then F-14.9, then M3 Story Intelligence (F-5.6 summaries first; F-14.11 beta reader rides on them). F-5.13 (job queue) stays queued. Read `PLAN.md` §2 and §3 before anything under §2.14.
