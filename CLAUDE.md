...
2. M3 is complete (2026-09-19). F-7.4 was retired into M4 (F-9) and F-7.3 ticked the same day, so the launch line is clear. M-Cloud started with F-15.2 (email magic link: `cloud/` Worker + D1, `src/main/account/`, the Account tab; Google/Apple split to F-15.12). Operator steps still open for F-15.2 (see the F-15.2 bullet in `docs/ARCHITECTURE.md`): the `.env` token needs Account → D1 → Edit, then `npm run cloud:migrate` and `npm run cloud:deploy`; a Resend account with `mythscribe.app` verified and `wrangler secret put RESEND_API_KEY` from `cloud/`. Next: F-15.3 credits, then F-15.4 proxy (read `PLAN.md` §3 and §4 first); F-15.4 reuses the Worker, the `Store`, and the bearer session from F-15.2.
