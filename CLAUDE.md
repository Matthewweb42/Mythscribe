...
2. M1 is under way: F-1.1 through F-1.5, F-2.1 through F-2.6, F-3.1 through F-3.3, and F-3.8 are done. Continue with `/feature F-3.4` (book-like column: one owner for the column width as a CSS variable F-3.6 will set), F-3.5 (empty state), F-3.6 (formatting settings: `settings:get`/`settings:set` channels for the `editor` key, live in the editor), F-3.7 (notes), then F-7.2, F-7.3, F-7.5 (editor tab) to close M1. v0 import was split out as F-1.6 (post-launch).
