...
2. M3 is complete (2026-09-19) and the launch line is clear (F-7.4 retired into M4/F-9). M-Cloud built: F-15.2 (magic link; Worker live at `api.mythscribe.app`), F-15.3 credits, F-15.4 proxy and the `cloud` adapter, F-15.5 usage meter, F-15.11 wizard AI source, F-15.7 signed builds and auto-update (2026-09-21: `electron-updater` on the GitHub releases feed, Settings → Updates, `.github/workflows/release.yml`). F-15.8 opt-in diagnostics (2026-09-21: Settings → Diagnostics, `src/main/diagnostics/`, Worker `POST /diagnostics`, D1 migration `0003_diagnostics`). F-15.6 was retired into F-15.9. Open operator steps: F-15.3 Lemon Squeezy store, packs, `LEMONSQUEEZY_WEBHOOK_SECRET`, `LEMONSQUEEZY_PACKS`, `npm run cloud:migrate` (recipe in `cloud/README.md`); F-15.4 `npx wrangler secret put OPENAI_API_KEY` from `cloud/`; then `npm run cloud:migrate` (also applies F-15.8's `0003_diagnostics`) and one `npm run cloud:deploy` (also ships F-15.5's `/credits` period fields and F-15.8's `/diagnostics`; until then the app's reports get a 404 and wait); F-15.7 the Azure Trusted Signing account, the Apple Developer ID, and the repository secrets (`docs/RELEASING.md`), then the first tag, the website's download buttons, and `npm run site:deploy` (the privacy page gained an update-check paragraph and F-15.8's diagnostics paragraph). Next: F-15.9 (which carries the signed license token).
