...
2. M1 is under way: F-1.1 through F-1.5, F-2.1 through F-2.4, F-3.1, and F-3.2 are done (`document:get`/`document:save` are the content channels; autosave lives in the renderer document store). Continue with `/feature F-2.5` together with F-3.8 (stacked scene editing; each region saves independently through the same store, whose pending jobs are already keyed per document), then F-2.6, and the rest of F-3.x in milestone order (F-3.3 word counts can reuse `wordCountRollup` and the `document:save` result). v0 import was split out as F-1.6 (post-launch).
