...
2. M1 is under way: F-1.1 through F-1.5, F-2.1 through F-2.4, and F-3.1 are done (the `node` table exists; `tree:create`/`tree:rename`/`tree:duplicate`/`tree:delete`/`tree:move` are the mutation channels; `document:get` is the only content channel, so the editor is read-path only and edits are discarded when the author switches documents). Continue with `/feature F-3.2` (autosave: `document:save`, debounce, Ctrl+S, word count on save, register with the pending-save registry, then make the smoke test assert the text after reopen), then F-2.5/F-3.8, F-2.6, and the rest of F-3.x in milestone order. v0 import was split out as F-1.6 (post-launch).
