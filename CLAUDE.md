...
2. M1 is under way: F-1.1 through F-1.5, F-2.1 through F-2.6, F-3.1 through F-3.6, and F-3.8 are done. Continue with `/feature F-3.7` (per-document notes: the `node.notes` column exists; `notes:get`/`notes:save` or extend the document channels; a resizable side panel next to the editor toggled from the toolbar, autosaving through the same document-store pattern), then F-7.2 (resizable panels; persist sizes in app state), F-7.3 (tabbed sidebar shell), F-7.5 (Settings dialog; move the Formatting popover's controls into its Editor tab) to close M1. v0 import was split out as F-1.6 (post-launch).
