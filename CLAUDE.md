...
1. Done: M0, M1, and all of M2 except F-7.4 (the spec marks it replaced by F-9 entities; decide with the user whether to build the throwaway or move it past the line). Also done from M2.5: F-14.1, F-14.4, F-14.5, F-14.6, F-14.7, F-14.8 (editor mode; the beta reader is F-14.11 in M3), F-14.10, F-5.4, F-5.10, F-5.12; F-5.5 and F-15.1 are reconciled as satisfied by F-14.4/F-5.3/F-5.2 and F-5.1/F-5.11. Per-feature status is in `FEATURES.md`; per-area notes are in `docs/ARCHITECTURE.md`.
2. Next: F-14.2 author rules and banned phrases (prompt constraints plus a post-filter; wire into ghost text, rewrite, and the critique fixes), then F-14.3 scene brief (replaces the notes-as-brief stand-in in `critique.v1` and ghost text), then F-14.9, then M3 Story Intelligence (F-5.6 summaries first; F-14.11 beta reader rides on them). F-5.13 (job queue) stays queued. Read `PLAN.md` §2 and §3 before anything under §2.14.
