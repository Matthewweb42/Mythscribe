...
2. M1 is under way: F-1.1 through F-1.5, F-2.1 through F-2.6, F-3.1 through F-3.8 are done. Remaining for M1: `/feature F-7.2` (resizable panel layout: sidebar and notes panel sizes persisted in app state; the notes panel's in-file ResizeObserver clamp is the hook-up point), F-7.3 (tabbed sidebar shell with the Manuscript tab functional), F-7.5 (Settings dialog with the Editor tab hosting the Formatting popover's controls). v0 import was split out as F-1.6 (post-launch).
...
- Autosave (F-3.2, F-3.7): `src/renderer/features/editor/autosaveStore.ts` is the one owner of the machinery: `createAutosaveStore<T, R>({ empty, get, save, onSaved?, delayMs? })` returns `{ useStore, reset }` with `docs[id] = { content, dirty }`, `load/unload/edit/flush/saveNow/clear`, per-id debounce timers and load tokens, pending jobs keyed per id, one serialized write queue, retry after failure unless a newer edit replaced the job, and its own pending-save registration (first load) and unregistration (clear/reset). `documentStore.ts` (`useDocumentStore`, `resetDocumentStore`, `onSaved` merges the word count into the tree) and `notesStore.ts` (`useNotesStore`, `resetNotesStore`) are its two instances. Notes: `src/main/document/notesStore.ts` (documents and folders; section roots refused by `parentId === null`; shares `parseStoredTiptap` with the document store); `NotesEditor.tsx` takes only `id`; `NotesPanel.tsx` (toggle button in both toolbars' `right` slot, resizable handle with `role="separator"`, width clamped in-file) and `notesPanelStore.ts` (open/width, in memory until F-7.2); `App.tsx` `MainPane` renders it beside the editor for documents and folders.
