...
2. M1 is nearly done: F-1.1 through F-1.5, F-2.1 through F-2.6, F-3.1 through F-3.8, and F-7.2 are done. Remaining for M1: `/feature F-7.3` (tabbed sidebar shell: a tab bar with a tab registry; Manuscript is the only functional tab until M2's Tags and later F-9.x/F-10.x/F-11.x register theirs, so no placeholder tabs), then F-7.5 (Settings dialog, Ctrl+, with the Editor tab hosting the Formatting popover's controls with live preview; the AI tab arrives with F-5.1). v0 import was split out as F-1.6 (post-launch).
