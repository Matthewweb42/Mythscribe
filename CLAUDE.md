...
2. F-5.6 scene summaries done and merged into main 2026-09-15; the feature worktree and its branches were removed the same day (only the `cloud-website` worktree at `~/coding/mythscribe-cloud` remains, and it is fully merged into main). Main runs a per-node debounced scheduler that F-5.13 grows into the real queue; the story bible carries the neighbours' summaries. F-14.11 beta reader done 2026-09-15 (reads the summaries in order; no voice block or bible by design). M3 remaining: F-5.7 query mode, F-5.8 rename modes, F-5.9 cost visibility, F-5.13 job queue, F-4.10 tag filter. Next: F-5.7; F-5.13 after. Read `PLAN.md` §2 and §3 before anything under §2.14.
