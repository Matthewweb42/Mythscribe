...
2. M1 is under way: F-1.1 through F-1.5, F-2.1 through F-2.6, F-3.1, F-3.2, and F-3.8 are done. Continue with `/feature F-3.3` (word counts: status bar under the editor with the document count and a session delta; folder views show the combined count; `wordCountRollup` and the `document:save` result already carry the numbers, so this is renderer-only), then F-3.4, F-3.5, F-3.6, F-3.7, then F-7.2, F-7.3, F-7.5 (editor tab) to close M1. v0 import was split out as F-1.6 (post-launch).
