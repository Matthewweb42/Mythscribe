...
1. Done: M0–M3 and M2.5 (2026-09-19); F-7.4 retired into M4/F-9. Per-feature status is in `FEATURES.md`; per-area notes are in `docs/ARCHITECTURE.md`.
2. M-Cloud is complete except F-15.12 (Google/Apple sign-in, post-launch): F-15.2 magic link (Worker live at `api.mythscribe.app`), F-15.3 credits, F-15.4 proxy, F-15.5 usage meter, F-15.11 wizard AI source, F-15.7 signed builds and auto-update (`.github/workflows/release.yml`), F-15.8 opt-in diagnostics (D1 `0003`), F-15.9 Supporter license (2026-09-21: Ed25519 token from `GET /license`, verified in main, 14-day grace; extras = badge + six accent colours; D1 `0004`; F-15.6 folded in). Open operator steps: F-15.3 Lemon Squeezy store, packs, `LEMONSQUEEZY_WEBHOOK_SECRET`, `LEMONSQUEEZY_PACKS` (recipe in `cloud/README.md`); F-15.4 `npx wrangler secret put OPENAI_API_KEY` from `cloud/`; F-15.9 `npm run cloud:license-keygen`, `wrangler secret put LICENSE_SIGNING_KEY`, paste the public JWK into `LICENSE_PUBLIC_KEY_JWK` in `src/shared/license.ts` (still the placeholder, so no build verifies a license yet), the $39 product and `LEMONSQUEEZY_SUPPORTER`; then `npm run cloud:migrate` (applies `0003_diagnostics` and `0004_supporter`) and one `npm run cloud:deploy` (ships F-15.5's `/credits` period fields, F-15.8's `/diagnostics`, and `/license`; until then the app gets 404s and waits); F-15.7 the Azure Trusted Signing account, the Apple Developer ID, and the repository secrets (`docs/RELEASING.md`), then the first tag, the website's download buttons, and `npm run site:deploy` (the privacy page gained update-check, diagnostics, and Supporter-license paragraphs). 2026-09-22, from the author's first real use: F-3.13 copy fix (built, unticked until the author's paste check), F-7.10 document zoom (Ctrl+= / − / 0, Ctrl+wheel) and interface size (Settings › Appearance) (built; a whole-window first cut was corrected the same day). Next: the operator steps above, then `/audit` before the first tag; post-launch code starts at M4 (`FEATURES.md` §6), with F-4.12/F-4.12b (automatic mentions and proposed tags) pulled first since the author asked for them.
