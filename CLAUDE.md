...
1. Done: M0, M1, and all of M2 except F-7.4 (the spec marks it replaced by F-9 entities; decide with the user whether to build the throwaway or move it past the line). Also done from M2.5: F-14.1, F-14.2, F-14.4, F-14.5, F-14.6, F-14.7, F-14.8 (editor mode; the beta reader is F-14.11 in M3), F-14.3, F-14.10, F-5.4, F-5.10, F-5.12; F-5.5 and F-15.1 are reconciled as satisfied by F-14.4/F-5.3/F-5.2 and F-5.1/F-5.11. Per-feature status is in `FEATURES.md`; per-area notes are in `docs/ARCHITECTURE.md`.
2. Next: merge F-14.9, already built on branch `worktree-f-14-9` (worktree `.claude/worktrees/f-14-9`, commit 01be530, another session, 2026-09-15). F-14.3 landed on `main` first, so that branch must rename its `<feature>.v2.ts` prompt files (tests, catalogue constants, fixture cases, token-report rows) to v3, rebase its cases on the F-14.3 v2 prompts, rerun `npm run eval:ai -- -u`, and resolve small conflicts in this section, `FEATURES.md`, `docs/ARCHITECTURE.md`, `src/shared/ai.ts`, `src/shared/aiSettings.ts`, and the site docs table. Then M3 Story Intelligence (F-5.6 summaries first; F-14.11 beta reader rides on them). F-5.13 (job queue) stays queued. Read `PLAN.md` §2 and §3 before anything under §2.14.
