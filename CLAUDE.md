...
2. M3 so far: F-5.6 scene summaries (2026-09-15; main runs a per-node debounced scheduler that F-5.13 grows into the real queue), F-14.11 beta reader (2026-09-15), F-5.7 query mode (built 2026-09-17: the `Query` mode of the assistant panel, lexical retrieval over summaries, verified citations; embeddings are still a later feature) is committed but **not ticked**: its e2e never ran because Electron crashes on WSLg's X11 path (Known gotchas). First thing next session: restart WSL (`wsl --shutdown` from Windows), run `npm run test:e2e`, tick F-5.7 if green, then start F-5.13. Only the `cloud-website` worktree at `~/coding/mythscribe-cloud` remains, fully merged into main. M3 remaining: F-5.13 job queue, F-5.8 rename modes (Query becomes the default), F-5.9 cost visibility, F-4.10 tag filter. Read `PLAN.md` §2 and §3 before anything under §2.14.
