...
2. M3 so far: F-5.6 scene summaries (2026-09-15; main runs a per-node debounced scheduler that F-5.13 grows into the real queue), F-14.11 beta reader (2026-09-15), F-5.7 query mode (2026-09-17: the `Query` mode of the assistant panel, lexical retrieval over summaries, verified citations; embeddings are still a later feature). Only the `cloud-website` worktree at `~/coding/mythscribe-cloud` remains, fully merged into main. M3 remaining, in order: F-5.13 job queue (next), F-5.8 rename modes (Query becomes the default), F-5.9 cost visibility, F-4.10 tag filter. Read `PLAN.md` §2 and §3 before anything under §2.14.
