...
2. F-14.9 story bible is done (2026-09-15; every prose prompt is now at v2 with the bible slot). F-14.3 scene brief was being built in a parallel session the same day (replaces the notes-as-brief stand-in in critique and ghost text; the voice block already carries the F-14.2 author rules): whichever of the two merges second renames its prompt files to the next version number and rebases its eval cases. Then M3 Story Intelligence (F-5.6 summaries first; F-14.11 beta reader rides on them). F-5.13 (job queue) stays queued. Read `PLAN.md` §2 and §3 before anything under §2.14.
