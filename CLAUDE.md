...
2. M1 is under way: F-1.1 through F-1.5 and F-2.1 through F-2.3 are done (the `node` table exists; `tree:create`/`tree:rename`/`tree:duplicate`/`tree:delete` are the mutation channels; the tree store owns selection, collapse, and rename state). Continue with `/feature F-2.4` (drag and drop; `deleteNode`'s gap-closing and `duplicateNode`'s sibling shift are the position-maintenance pattern a move needs), then F-2.5–2.6 and the editor (F-3.x) in milestone order. v0 import was split out as F-1.6 (post-launch).
...
- Tree mutations (pattern for F-2.4): main validates structure inside a transaction in `src/main/tree/treeStore.ts` (`createNode`, `renameNode`, `duplicateNode`, `deleteNode`; section roots are refused with `VALIDATION`, sibling positions stay contiguous) and returns the row(s); the renderer store merges the returned records with pure, tested helpers (`insertIntoIndex`, `duplicateIntoIndex`, `removeFromIndex`, `planRemoval` for the post-delete selection fallback) instead of reloading; `expandAncestors` opens the path to a new node. Delete confirms through `dialogs.confirm` in `ManuscriptTree.tsx`. `placement.ts` is the one place that turns "selection + level" into `{ parentId, afterId }` (bottom bar, context menu, and later Insert menu/shortcuts all use it). `contextMenuItems.ts` returns plain `{ id, label }` data so F-7.1 can render the same items natively; F-2.6 appends its template group there. Default titles come from `defaultNodeTitle` in `src/shared/labels.ts`.
