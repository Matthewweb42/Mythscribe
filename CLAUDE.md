...
2. M3 so far: F-5.6 scene summaries (2026-09-15), F-14.11 beta reader (2026-09-15), F-5.7 query mode (2026-09-17: the `Query` mode of the assistant panel, lexical retrieval over summaries, verified citations; embeddings are still a later feature), F-5.13 background job queue (2026-09-17: migration `0007_index_jobs`, `src/main/jobs/{jobStore,indexQueue}.ts` in place of the F-5.6 summary scheduler, the four `jobs:*` channels and the `jobs:changed` event, the header `IndexingIndicator`, and "Summarize all scenes" in the AI tab; the spec is amended — no provider batch API at launch, and only `summary` jobs exist). Only the `cloud-website` worktree at `~/coding/mythscribe-cloud` remains, fully merged into main. F-5.8 rename modes (2026-09-17: `CHAT_MODES` order query/agent/plan, labels Query/Author/Plan, stored values unchanged, Query is the default for a fresh conversation). F-5.9 cost visibility (2026-09-17: `describeRequest` on every cost line, `ChatMessage.usage`, the in-memory session tally in `src/main/ai/sessionUsage.ts`, `session` and `recent` on `ai:usageSummary`; per-task model choice is the per-tier setting of F-5.11). M3 remaining: F-4.10 tag filter (next), the last M3 feature. Read `PLAN.md` §2 and §3 before anything under §2.14.
