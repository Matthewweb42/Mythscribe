...
1. Done: M0, M1, and all of M2 except F-7.4 (the spec marks it replaced by F-9 entities; decide with the user whether to build the throwaway or move it past the line). Also done from M2.5: F-14.1, F-14.4, F-14.5, F-14.6, F-14.7, F-5.4, F-5.10, F-5.12; F-5.5 and F-15.1 are reconciled as satisfied by F-14.4/F-5.3/F-5.2 and F-5.1/F-5.11. Per-feature status is in `FEATURES.md`; per-area notes are in `docs/ARCHITECTURE.md`.
2. Next: F-14.10 rewrite in my voice (the first diff rendering), then F-14.8 critique, F-14.2/14.3/14.9 context, then M3 Story Intelligence. F-5.13 (job queue) stays queued. Read `PLAN.md` §2 and §3 before anything under §2.14.
