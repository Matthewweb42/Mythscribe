...
- [ ] **F-7.5 Settings dialog** (v0 ✅; M1: Editor tab built, unit-tested, e2e verified via Ctrl+, and the header button, with the F-3.6 controls and a live preview; the tab list is a registry, so the AI tab arrives with F-5.1) — Modal with Editor and AI tabs (Ctrl+,). Editor: F-3.6 with live preview. AI: key/connection, enable toggle, presets, behavior.
