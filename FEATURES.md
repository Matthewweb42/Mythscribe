...
- [ ] **F-4.4 Document tag bar** (v0 ✅; M2: part built, unit-tested, e2e verified: linked-tag chips with remove, "Add tag" picker of the unassigned tags (search, ↑/↓, Enter, Esc), collapse, and a persisted draggable height 100 px–60 vh through `documentTag:list/add/remove` and the `tagBar` layout field; the inline-tag occurrence counts arrive with F-4.6, the metadata split with F-4.5) — Bar above the editor showing the document's tags as colored chips with remove buttons; "Add" opens a searchable picker of unassigned tags; inline tags used in the content are listed with occurrence counts. Bar is collapsible, has a draggable horizontal split between metadata and tags (30–70 %) and a draggable height (100 px–60 vh); layout persists.
