...
/**
 * The assistant prompt (F-5.4), version 1, for both modes. Prompt files are versioned
 * (F-5.12): a change to the text, the caps, or the message order is a new file (`chat.v2.ts`)
 * with its own golden test, never an edit here, so every ledger row's `promptVersion` stays true.
 *
 ...
 * notes, the scene metadata (Agent mode), the scene's text. The context sits at the end of the
 * system turn so the stable part in front of it caches at the provider, and so a multi-turn
 * conversation about one scene reuses the whole system turn. The history turns follow as real
 * user/assistant messages, the author's message last.
 */
