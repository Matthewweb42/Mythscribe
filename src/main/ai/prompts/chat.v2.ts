...
/**
 * The assistant prompt (F-5.4), version 2, for both modes. Changed from `chat.v1` (F-14.9):
 * the system turn carries the story bible block (`renderStoryBible`) between the rules and
 * the context, in both modes — Plan mode answers about the manuscript need the same ground
 * truth (PLAN.md §2.3). Nothing else moved. Prompt files are versioned (F-5.12): a change to
 * the text, the caps, or the message order is a new file (`chat.v3.ts`) with its own golden
 * test, never an edit here, so every ledger row's `promptVersion` stays true.
 *
 ...
 * cheapest first: the referenced notes, the scene metadata (Agent mode), the scene's text. The
 * context sits at the end of the system turn so the stable part in front of it caches at the
 * provider, and so a multi-turn conversation about one scene reuses the whole system turn. The
 * history turns follow as real user/assistant messages, the author's message last.
 */
...
export interface BuiltChatPrompt {
  version: typeof CHAT_PROMPT_VERSION
  messages: AiMessage[]
  maxTokens: number
