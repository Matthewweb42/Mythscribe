...
/**
 * The Agent-mode regenerate prompt (F-5.4, F-14.7), version 2: `chat.v2` with one clause
 * appended to the system turn naming what the fidelity check found wrong with the first
 * answer. Changed from `chatRegen.v1` (F-14.9) only in the base prompt it wraps, which now
 * carries the story bible; everything else (the clause, the voice block, the preset, the
 * context, the history, the caps) is the same, so the provider's cached prefix still applies
 * up to the clause. A change to the clause or the base prompt is a new file with its own
 * golden test, never an edit here.
 */
export const CHAT_REGEN_PROMPT_VERSION = 'chatRegen.v2'
