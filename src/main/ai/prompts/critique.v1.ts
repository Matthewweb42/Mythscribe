...
/**
 * The editor's-notes prompt (F-14.8), version 1: one scene read as an editor would read it,
 * answered as JSON notes that each cite a passage, with an optional fix for the quoted
 * passage only. Prompt files are versioned (F-5.12): a change to the text, the caps, or the
 * message order is a new file (`critique.v2.ts`) with its own golden test, never an edit
 ...
 * (F-14.1), and the scene's metadata line, so the provider's prefix cache still applies when
 * the author asks again on the same scene. The user turn carries what changes: the author's
 * notes as the scene's brief (the stand-in until F-14.3) and the scene text itself.
 */
