...
import { buildCritiquePrompt as buildV1 } from './critique.v1'
import {
  buildCritiquePrompt,
  CRITIQUE_RULES,
  HONESTY_INSTRUCTION,
...
describe('critique.v2 prompt (F-14.8, F-14.9)', () => {
  ...
  it('F-14.9: a null bible leaves the v1 messages exactly', () => {
    ...
    expect(buildCritiquePrompt(input).messages).toEqual(buildV1(input).messages)
  })
  ...
  it('stays under the critique input budget with every cap at its limit, the bible included', () => {
    const maxed: BuildCritiquePromptInput = {
    ...
    const estimate = estimateTokens(promptText(buildCritiquePrompt(maxed).messages))
    ...
    expect(estimate).toBe(6_646)
    expect(
      estimateTokens(promptText(buildCritiquePrompt({ ...maxed, bible: null }).messages))
    ).toBe(6_245)
