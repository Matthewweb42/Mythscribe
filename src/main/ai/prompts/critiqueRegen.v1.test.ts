...
import { buildCritiqueRegenPrompt } from './critiqueRegen.v1'
...
describe('critiqueRegen.v1 prompt (F-14.8, F-14.5)', () => {
  it('is critique.v1 with the author’s note appended to the system turn, the user turn untouched', () => {
    const built = buildCritiqueRegenPrompt({ ...base, note: NOTE })
    const plain = buildCritiquePrompt(base)
    expect(built.version).toBe('critiqueRegen.v1')
