...
/**
 * The editor's-notes regenerate prompt (F-14.8), version 2: `critique.v2` with the author's
 ...
 * Changed from `critiqueRegen.v1` (F-14.9) only in the base prompt it wraps, which now
 * carries the story bible; everything else (the honesty line, the voice block, the scene
 * line, the notes, the scene text, the cap) is the same, so the provider's cached prefix
 * still applies up to the clause. A change to the clause or the base prompt is a new file
 * with its own golden test, never an edit here.
 */
export const CRITIQUE_REGEN_PROMPT_VERSION = 'critiqueRegen.v2'
