...
/**
 * The ghost-text continuation prompt (F-5.3), version 2: version 1 plus the scene brief
 * (F-14.3) — the scene's own five lines with the previous scene's reader-knows-after line and
 * the next scene's goal — so a continuation is written toward what the author said the scene
 * is for. Prompt files are versioned (F-5.12): a change to the text, the caps, or the message
 * order is a new file with its own golden test, never an edit to a shipped one, so every
 * ledger row's `promptVersion` stays true; `ghostText.v1.ts` stays exactly as it shipped.
 *
 ...
 * new-elements rule (F-5.2). The user turn carries the task-specific context, cheapest first:
 * the scene's metadata, its brief, and its notes, then the caret window, then the one-line
 * instruction.
 */
export const GHOST_PROMPT_V2_VERSION = 'ghostText.v2'
...
export interface BuildGhostTextPromptV2Input {
...
export interface BuiltGhostTextPromptV2 {
  version: typeof GHOST_PROMPT_V2_VERSION
  messages: AiMessage[]
  maxTokens: number
  temperature: number
}

export function buildGhostTextPromptV2(input: BuildGhostTextPromptV2Input): BuiltGhostTextPromptV2 {
  ...
  return {
    version: GHOST_PROMPT_V2_VERSION,
    messages: [
