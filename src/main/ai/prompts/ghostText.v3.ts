...
/**
 * The ghost-text continuation prompt (F-5.3), version 3: version 2 (the scene brief, F-14.3)
 * plus the story bible (F-14.9): the system turn closes with the bible block
 * (`renderStoryBible`) after the rules, the voice profile, and the preset, as the ground truth
 * for names and places. Nothing else moved. Prompt files are versioned (F-5.12): a change to
 * the text, the caps, or the message order is a new file with its own golden test, never an
 * edit to a shipped one, so every ledger row's `promptVersion` stays true; `ghostText.v2.ts`
 ...
 * new-elements rule (F-5.2), then the story bible (F-14.9), which holds still while the author
 * writes the scene. The user turn carries the task-specific context, cheapest first: the
 * scene's metadata, its brief, and its notes, then the caret window, then the one-line
 * instruction.
 */
export const GHOST_PROMPT_V3_VERSION = 'ghostText.v3'
...
export interface BuildGhostTextPromptV3Input {
  ...
  /**
   * The scene brief block (F-14.3, `renderSceneBriefBlock` through `sceneBriefBlock`), or null
   * when neither the scene nor its neighbours have one.
   */
  brief: string | null
  /**
...
export interface BuiltGhostTextPromptV3 {
  version: typeof GHOST_PROMPT_V3_VERSION
  messages: AiMessage[]
  maxTokens: number
  temperature: number
}

export function buildGhostTextPromptV3(input: BuildGhostTextPromptV3Input): BuiltGhostTextPromptV3 {
  ...
  return {
    version: GHOST_PROMPT_V3_VERSION,
    messages: [
