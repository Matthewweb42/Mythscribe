...
import { buildGhostTextRegenPrompt as buildRegenV1 } from './ghostTextRegen.v1'
import { buildGhostTextRegenPrompt, REGEN_CLAUSE_PREFIX } from './ghostTextRegen.v2'
...
describe('ghostTextRegen.v2 prompt (F-14.7, F-14.9)', () => {
  ...
  it('F-14.9: keeps the story bible ahead of the clause too', () => {
    const built = buildGhostTextRegenPrompt({ ...minimal, bible: BIBLE, violation: VIOLATION })
    const system = built.messages[0]?.content ?? ''
    expect(system.indexOf(STORY_BIBLE_HEADING)).toBeGreaterThan(
      system.indexOf(general.styleInstruction)
    )
  ...
  it('F-14.9: a null bible leaves the v1 messages exactly', () => {
    const input = { ...minimal, voice: 'Short sentences; never semicolons.', violation: VIOLATION }
    expect(buildGhostTextRegenPrompt(input).messages).toEqual(buildRegenV1(input).messages)
  })
  ...
  it('stays under the ghost-text input budget for a maxed-out window, notes, metadata, and bible', () => {
    ...
    // The golden estimate: the base prompt's 858 plus the clause.
    expect(estimate).toBe(892)
  })
})
