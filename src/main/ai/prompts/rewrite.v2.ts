...
/**
 * The rewrite-in-my-voice prompt (F-14.10), version 2: one selected passage rewritten in the
 * author's voice, shown to the author as a diff. Changed from `rewrite.v1` (F-14.9): the
 * system turn carries the story bible block (`renderStoryBible`) between the voice profile
 * and the scene's metadata line, so the rewrite keeps the names and facts the bible states.
 * Nothing else moved. Prompt files are versioned (F-5.12): a change to the text, the caps, or
 * the message order is a new file (`rewrite.v3.ts`) with its own golden test, never an edit
