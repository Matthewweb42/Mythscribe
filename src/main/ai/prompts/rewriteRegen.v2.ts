...
/**
 * The rewrite regenerate prompt (F-14.10), version 2: `rewrite.v2` with up to two clauses
 ...
 * instruction and the violation is only a correction. Changed from `rewriteRegen.v1`
 * (F-14.9) only in the base prompt it wraps, which now carries the story bible; everything
 * else (the clauses, the voice block, the scene line, the context, the passage, the cap) is
 * the same, so the provider's cached prefix still applies up to the clauses. A change to a
 * clause or the base prompt is a new file with its own golden test, never an edit here.
 */
export const REWRITE_REGEN_PROMPT_VERSION = 'rewriteRegen.v2'
