...
describe('App', () => {
  it('shows the welcome screen, creates a project through the prompt, then closes it', async () => {
    const invoke = install({ 'project:create': info })
    render(<App />)
    await userEvent.click(await screen.findByRole('button', { name: /new project/i }))
    await userEvent.type(
      await screen.findByRole('textbox', { name: 'New project' }),
      'Smoke{Enter}'
    )
